...
const app = express();
const conString = `postgres://postgres:${process.env.PG_PASSWORD}@localhost:5432/`; // DONE:Don't forget to set your own conString
...
app.get('/articles', function(request, response) {
  ...
  client.query(`SELECT * FROM articles INNER JOIN authors ON articles.author_id = author.author_id`)
...
app.post('/articles', function(req, res) {
  client.query(
    'INSERT INTO authors (author_id, author, "authorUrl") VALUES ($1, $2, $3) ON CONFLICT DO NOTHING;', // DONE: Write a SQL query to insert a new author, ON CONFLICT DO NOTHING
    [
      req.body.author_id,
      req.body.author,
      req.body.authorUrl
  ...
  function queryTwo() {
    client.query(
      `SELECT author_id FROM authors WHERE author = $2`, // DONE: Write a SQL query to retrieve the author_id from the authors table for the new article
      [req.body.author], // DONE: Add the author name as data for the SQL query
      function(err, result) {
        if (err) console.error(err)
  ...
  function queryThree(author_id) {
    client.query(
      `INSERT INTO articles (article_id, title, category,  "publishedOn", body, author_id) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT DO NOTHING RETURNING *;`, // DONE: Write a SQL query to insert the new article using the author_id from our previous query
      [
        req.body.article_id,
        req.body.title,
        req.body.category,
